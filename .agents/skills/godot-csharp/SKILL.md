...
name: godot-csharp
description: >
  Write or debug Godot 4.6.2 C#/.NET code in .cs or .csproj files, including lifecycle
  overrides, [Export], [Signal], node lookup, and GDScript interop. Use only for the
  Godot .NET build; use godot-gdscript for .gd syntax and the relevant domain Skill
  for language-neutral scenes, physics, animation, or resources.
---

# Godot C# / .NET (4.x)

Write Godot game code in C#: node subclasses, the engine lifecycle, exports, signals as
events, and GDScript interop. Targets **Godot 4.6.2 (.NET / C#)** with **.NET 8 or later**.
...
1. **Use the Godot .NET editor build** and install a compatible .NET 8 or later SDK. Creating the first C#
...
- **Needs the .NET build + SDK.** The non-.NET editor can't run C#; mismatched/missing
  .NET SDK causes build failures. Godot 4.6.2 requires .NET 8 or later; check current platform
