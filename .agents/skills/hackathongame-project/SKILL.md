...
- Treat `EventBus.emit()` as synchronous snapshot dispatch: validate the dynamic `Dictionary` payload and account for listener order when same-call-stack completion matters. Use `emit_deferred()` only for an explicit next-process-frame boundary; transition cleanup cancels pending transient events.
- Treat `Global/UILayerContract.gd` as the only source for formal `CanvasLayer.layer` and shared-HUD `z_index` values. Add level-owned gameplay controls through the HUD gameplay container instead of attaching parallel HUD roots or inventing numeric layers.
- Pair modal and cinematic pause suppression with owner-scoped `InputManager.acquire_pause_guard()` tokens. A pause guard blocks `ui_pause` without swallowing a modal's own close/confirm input; use a full input lock only when gameplay input must also stop.
- Fullscreen gameplay UI that needs pointer interaction must acquire an owner-scoped `InputManager.acquire_pointer_release()` token and release that exact token when it closes. Nested overlays keep independent tokens; never write `Input.mouse_mode` outside `InputManager`.
- Keep transition-only teardown in `prepare_for_level_exit()`. `_exit_tree()` should release only node-owned connections, callbacks, guards, and resources; do not broaden it into a second global transition cleanup path.
