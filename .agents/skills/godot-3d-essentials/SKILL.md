...
description: >
  Set up Godot 4.6.2 3D scenes with Node3D transforms, Camera3D, lights,
  WorldEnvironment, MeshInstance3D materials, or GridMap. Use for 3D scene and
  environment assembly; use godot-physics for bodies or raycasts, godot-shaders
  for shader code, and godot-animation for character animation graphs.
---

# Godot 3D Essentials (4.x)

Assemble a working 3D scene: transforms, camera, lights, environment/post, materials, and
`GridMap` blockouts. Targets **Godot 4.6.2**.
