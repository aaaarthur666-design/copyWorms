...
name: godot-tilemap
description: >
  Build or debug Godot 4.6.2 tile-based 2D levels with TileMapLayer, TileSet,
  terrains, tile collision, navigation, custom data, or cell APIs. Use for tile
  authoring and runtime cells; use godot-2d-movement for character motion,
  godot-physics for general bodies or raycasts, and map-generation guidance for algorithms.
---

# Godot TileMap (4.6.2 TileMapLayer)
...
**Godot 4.6.2**, where `TileMapLayer` replaces the now-deprecated `TileMap` node.
