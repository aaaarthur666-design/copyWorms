...
description: >
  Implement or debug Godot 4.6.2 high-level network multiplayer with
  ENetMultiplayerPeer, @rpc, authority, MultiplayerSpawner, or
  MultiplayerSynchronizer. Use for networked peer replication; do not use for
  local split-screen, HTTP or raw socket protocols, or export-only tasks.
---

# Godot Multiplayer (4.x high-level)

Connect peers, call functions remotely with `@rpc`, assign authority, and replicate state
with `MultiplayerSpawner`/`MultiplayerSynchronizer`. Targets **Godot 4.6.2** (ENet). Treat
