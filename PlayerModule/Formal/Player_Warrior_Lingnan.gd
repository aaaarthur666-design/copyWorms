...
@export var lingnan_config: LingnanPlayerConfig = preload("res://DataConfig/Player/LingnanPlayerConfig.tres")
...
var _dash_attack_speed: float = 0.0
var _dash_attack_duration: float = 0.0
...
var _spin_max_hits: int = 0
var _spin_range: float = 0.0
var _spin_damage: int = 0
...
func _on_ready():
	super._on_ready()
	if not lingnan_config:
		push_error("[Player_Warrior_Lingnan] LingnanPlayerConfig.tres 加载失败，使用安全默认值")
		lingnan_config = LingnanPlayerConfig.new()
	_dash_attack_speed = lingnan_config.dash_speed
	_dash_attack_duration = lingnan_config.dash_duration
	_anim_map = {
...
func _on_physics_process(delta: float) -> void:
	...
	if not input_blocked and can_attack_hold_dash and not _is_charging and not _is_dashing_attack and not _is_spinning and not _dash_windup and not is_attacking and not is_dashing:
		if Input.is_action_pressed("player_attack") and _dash_attack_cd_timer <= 0:
			_attack_hold_time += delta
			if _attack_hold_time >= lingnan_config.attack_hold_threshold:
	...
	if _dash_windup:
		if input_blocked:
			velocity.x = move_toward(velocity.x, 0, lingnan_config.action_deceleration * delta)
			return
		...
		# 蓄力视觉：震动效果（随蓄力时长增强）
		if _anim_sprite:
			var charge_ratio = clampf(1.0 - _dash_windup_timer / lingnan_config.dash_windup_time, 0.0, 1.0)
		...
		if _anim_sprite and _anim_sprite.sprite_frames:
			if _anim_sprite.sprite_frames.has_animation("attack"):
				...
				_anim_sprite.pause()
		# 蓄力期间减速
		velocity.x = move_toward(velocity.x, 0, lingnan_config.action_deceleration)
		# 最少蓄力时间过后，玩家松开普攻键 → 突进
	...
	if _is_charging:
		...
		if not _charging_skill_2 and _charge_time >= lingnan_config.charge_threshold_big:
		# 蓄力超阈值后进入霸体
		...
		if _anim_sprite:
			...
			_anim_sprite.position = Vector2(shake_x, shake_y)
		# 蓄力临界点闪白（小/大回旋斩分界）
		if _charge_time >= lingnan_config.charge_threshold_big and _charge_time - delta < lingnan_config.charge_threshold_big:
			if _anim_sprite:
	...
	if _is_dashing_attack:
		...
		if _afterimage_cooldown <= 0:
			_spawn_afterimage()
			_afterimage_cooldown = lingnan_config.dash_afterimage_interval
	...
	if _is_spinning:
		...
		# 两段判定间隔
		if _spin_max_hits == lingnan_config.big_spin_hits and _spin_hit_count == 1 and _spin_timer <= lingnan_config.spin_second_hit_time:
			_do_spin_hit()
		if _spin_timer <= 0:
			_end_spin()
		return
...
func _get_active_charge_max_time() -> float:
	return lingnan_config.bagua_charge_max_time if _charging_skill_2 else lingnan_config.charge_max_time
...
func _release_skill() -> void:
	...
	var charge_ratio := clampf(_charge_time / (lingnan_config.bagua_charge_max_time if was_skill_2 else lingnan_config.charge_max_time), 0.0, 1.0)
	if _anim_sprite:
		_anim_sprite.scale = Vector2.ONE
		_anim_sprite.modulate = Color.WHITE
		_anim_sprite.position = Vector2.ZERO  # 恢复震动偏移
	if was_skill_2:
		_skill2_cooldown_timer = lingnan_config.skill_cooldown
		_do_bagua_shockwave(charge_ratio)
		_activate_bagua_shield()
	elif _charge_time < lingnan_config.charge_threshold_short:
		_skill_cooldown_timer = lingnan_config.skill_cooldown
		# 短按 → 小回旋斩
		_do_spin_slash(false)
	elif _charge_time < lingnan_config.charge_threshold_big:
		_skill_cooldown_timer = lingnan_config.skill_cooldown
		# 短蓄力 → 小回旋斩
		_do_spin_slash(false)
	else:
...
func _do_bagua_shockwave(charge_ratio: float) -> void:
	...
	attack_timer = lingnan_config.bagua_action_duration
	_change_state(GlobalDefine.PlayerState.SKILL)
...
func _push_enemy_from_bagua(enemy: Node2D, push_force: float, dist: float, shock_range: float) -> void:
	...
	if push_dir == Vector2.ZERO:
		push_dir = Vector2(1.0 if is_facing_right else -1.0, config.attack_direction_y_bias).normalized()
	var falloff := clampf(1.0 - dist / shock_range, lingnan_config.bagua_min_falloff, 1.0)
	var final_force := push_force * falloff
	if enemy is CharacterBody2D:
		var body := enemy as CharacterBody2D
		body.velocity = Vector2(push_dir.x * final_force, -final_force * lingnan_config.bagua_vertical_push_ratio)
		if "stun_timer" in body:
			body.set("stun_timer", maxf(float(body.get("stun_timer")), lingnan_config.bagua_enemy_stun_time))
	else:
...
func _activate_bagua_shield() -> void:
	_bagua_shield_hp = lingnan_config.bagua_shield_max_hp
...
func _start_dash_windup() -> void:
	_dash_windup = true
	_dash_windup_timer = lingnan_config.dash_windup_time
...
func _do_dash_attack() -> void:
	...
	_dash_attack_cd_timer = lingnan_config.dash_cooldown  # 突进独立CD
	is_attacking = true
	attack_timer = _dash_attack_duration + lingnan_config.dash_action_time_bonus
	# 突进期间无敌
	_dash_was_invincible = is_invincible
	is_invincible = true
	invincible_timer = _dash_attack_duration + lingnan_config.dash_action_time_bonus + lingnan_config.dash_invincible_bonus
	_change_state(GlobalDefine.PlayerState.ATTACK)
...
func _end_dash_attack() -> void:
	...
	for enemy in GameManager.get_enemies():
		...
		if dist <= lingnan_config.dash_path_hit_radius:
			_deal_dash_damage(enemy)
	# 震屏
	var cam = get_node_or_null("SmoothCamera")
	if cam and cam.has_method("shake"):
		cam.shake(lingnan_config.dash_camera_shake_strength, lingnan_config.dash_camera_shake_duration)

func _deal_dash_damage(enemy: Node2D) -> void:
	_dash_attack_hit_enemies.append(enemy.get_instance_id())
	var result = DamageCalculator.calculate(lingnan_config.dash_damage, 0, lingnan_config.dash_damage_type, lingnan_config.dash_crit_chance)
...
func _do_spin_slash(is_big: bool) -> void:
	...
	if is_big:
		_spin_timer = lingnan_config.big_spin_duration
		_spin_max_hits = lingnan_config.big_spin_hits
		_spin_range = lingnan_config.big_spin_range
		_spin_damage = lingnan_config.big_spin_damage
	else:
		_spin_timer = lingnan_config.small_spin_duration
		_spin_max_hits = lingnan_config.small_spin_hits
		_spin_range = lingnan_config.small_spin_range
		_spin_damage = lingnan_config.small_spin_damage

	attack_timer = _spin_timer + lingnan_config.spin_action_time_bonus
	_change_state(GlobalDefine.PlayerState.SKILL)
	...
	var cam = get_node_or_null("SmoothCamera")
	if cam and cam.has_method("shake"):
		cam.shake(lingnan_config.big_spin_camera_shake if is_big else lingnan_config.small_spin_camera_shake, lingnan_config.spin_camera_shake_duration)
...
func _do_spin_hit() -> void:
	...
	for enemy in GameManager.get_enemies():
		...
		if global_position.distance_to(enemy.global_position) <= _spin_range:
			...
			var result = DamageCalculator.calculate(_spin_damage, 0, lingnan_config.spin_damage_type, lingnan_config.spin_crit_chance)
			if enemy.has_method("take_damage"):
				enemy.take_damage(result["damage"], kb_dir)
			EventBus.emit(GlobalDefine.EventName.PLAYER_ATTACK_HIT, {"attacker": self, "target": enemy, "damage": result["damage"], "is_crit": result["is_crit"]})
	_spin_hit_count += 1
...
func _suck_enemies(delta: float) -> void:
	for enemy in GameManager.get_enemies():
		if not is_instance_valid(enemy):
			continue
		var dist = global_position.distance_to(enemy.global_position)
		if dist < lingnan_config.suck_range and dist > lingnan_config.suck_min_distance:
			var suck_dir = (global_position - enemy.global_position).normalized()
			# 吸附力随距离衰减（越近吸力越小，避免抖动）
			var strength = lingnan_config.suck_force * clampf(1.0 - dist / lingnan_config.suck_range, lingnan_config.suck_min_strength_ratio, 1.0)
			var pull_velocity = Vector2(suck_dir.x * strength, suck_dir.y * strength * lingnan_config.suck_vertical_ratio)
			var pull_motion = pull_velocity * delta
			if enemy is CharacterBody2D:
				var body := enemy as CharacterBody2D
				body.velocity.x = move_toward(body.velocity.x, pull_velocity.x, strength * delta * lingnan_config.suck_horizontal_response)
				body.velocity.y = move_toward(body.velocity.y, pull_velocity.y, strength * delta * lingnan_config.suck_vertical_response)
