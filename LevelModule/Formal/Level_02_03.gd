...
func _enter_ide_chat() -> void:
	...
	if _chat_input:
		_chat_input.text = ""
		_chat_input.placeholder_text = "Press Enter to Confirm Dialogue..."
...
func _render_next_chat_line() -> void:
	...
	_pending_chat_text = "[color=white]Ming: " + text + "[/color]\n"
...
func _enter_free_chat() -> void:
	...
	if _chat_input:
		_chat_input.text = ""
		_chat_input.editable = true
		_chat_input.placeholder_text = "Type a message and press Enter to send..."
		_chat_input.grab_focus()
...
func _on_chat_submitted(text: String) -> void:
	...
	if current_state == LevelState.REALITY_FREE_CHAT:
		...
		if msg == "/memory":
			...
			if target_area == 0:
				if _chat_window:
					_chat_window.append_text("[color=cyan]CodeBuddy: Childhood memory samples complete. Enter /config to continue.[/color]\n")
				if _chat_input:
					_chat_input.text = ""
				return
			if _chat_window:
				_chat_window.append_text("[color=white][b]Ming:[/b][/color] %s\n" % msg)
...
func _generate_ai_reply(user_msg: String) -> String:
	...
	if msg_lower.contains("爷爷") or msg_lower.contains("grandfather"):
		return "The “Grandpa” in your description has been designated as the core emotional anchor.\n\nPlease note: objects in the dream are reconstructed from memories and data. They are not the same as their real-world counterparts."
	if msg_lower.contains("梦") and (msg_lower.contains("醒") or msg_lower.contains("逃") or msg_lower.contains("出") or msg_lower.contains("回")):
		return "The current dream supports manual exit.\n\nHowever, after your recent decision to block external signals, the exit path may be compromised.\nProceed with caution."
	if msg_lower.contains("裂缝") or msg_lower.contains("断崖") or msg_lower.contains("跳"):
		return "The rift is the result of environmental tearing.\n\nIncreasing Base_Jump_Height should theoretically allow you to cross it.\nPlease confirm that recompilation is complete."
	if msg_lower.contains("黑影") or msg_lower.contains("敌人") or msg_lower.contains("伤害") or msg_lower.contains("打"):
		return "The shadows were generated by contamination from real-world anxiety data.\n\nOnce Player_Damage_Reduction is enabled, they will be unable to inflict meaningful harm."
	if msg_lower.contains("凉茶") or msg_lower.contains("铺"):
		return "The herbal tea shop lies deep within the dream.\n\nAccording to your memories, it is the center of “home” and “safety.”\nIt is also the most stable and most dangerous area in this project."
	if msg_lower.contains("帮助") or msg_lower.contains("help") or msg_lower.contains("怎么"):
		return "Enter /config to modify the dream configuration.\n\nAfter making all three changes, click “Recompile and Inject into Dream.”\nIf you experience discomfort, try to exit.\nAssuming the exit still exists."

	# 默认回复
	var defaults: Array[String] = [
		"I'm listening.\nWhat do you see on this old street?",
		"Ming, your heart rate has risen slightly.\nTake a deep breath.\nThis dream will not hurt you.\nAt least, not yet.",
		"This dream was compiled from memories of you and your grandpa on Xiguan Old Street.\nEvery granite slab and every Manchu window comes from your childhood.",
		"I understand.\nMemories always carry weight.\nEspecially on nights when you are afraid to look back.",
		"You must modify the configuration and recompile before you can gain power in this dream.\nEnter /config to begin configuration.",
...
func _run_recompile_sequence() -> void:
	...
	_show_narrative(level_data.compile_success_text if level_data else "Compilation complete.", func():
		_unlock_reality_bed()
	)
...
func _build_all_ui() -> void:
	...
	_wake_hint_label.text = "Hold [Tab] to Open Your Eyes"
	...
	archive_button.text = "> Lingnan Dream Compendium · Childhood Memories"
	...
	ctitle.name = "ConfigTitle"; ctitle.text = "[+] Xiguan_Dream.ini - Configuration Editor"
	...
	for i in range(3):
		...
		var btn = Button.new()
		btn.name = "ModifyButton_%d" % i; btn.text = "Modify"
	...
	_recompile_button.name = "RecompileButton"; _recompile_button.text = "Recompile and Inject into Dream"
